...
def fit(fn_name: str) -> dict:
    ...
    return ({"width": "stretch"} if "width" in signature(fn).parameters
...
def sidebar() -> tuple[pd.DataFrame, str, dict]:
    ...
    if provider == "Ollama (local)":
        cfg["base_url"] = sb.text_input("Ollama URL", "http://localhost:11434")
        cfg["model"] = sb.text_input("Model", "llama3.1")
    elif provider == "OpenAI-compatible":
        cfg["base_url"] = sb.text_input("Base URL", "https://api.openai.com/v1")
        cfg["model"] = sb.text_input("Model", "gpt-4o-mini")
        cfg["api_key"] = sb.text_input("API key", type="password")
    elif provider == "Anthropic":
        cfg["model"] = sb.text_input("Model", "claude-sonnet-4-5")
        cfg["api_key"] = sb.text_input("API key", type="password")
    else:
...
# =========================================================================== #
    ...
    if c1.button("Save journal to disk", type="primary"):
        p = storage.save(st.session_state.trades)
